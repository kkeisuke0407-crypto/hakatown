/* 「やることは、5つだけ。」の操作画面を、スクロールに合わせて動かす。
   - スクロール量をそのまま演出の時間に変える（戻せば巻き戻る。自動再生はしない）
   - JSが動かない環境では、HTMLに書いた STEP 1〜5 の文章がそのまま読める
   - 動きを減らす設定の人には、指の動きや画面送りを出さず、STEPごとに切り替えるだけにする */
(function () {
  var root = document.querySelector('[data-scrub]');
  if (!root || !('IntersectionObserver' in window) || !window.CSS || !CSS.supports('position', 'sticky')) return;

  var BASE = new URL('./images/', document.currentScript.src).href;
  var REDUCE = matchMedia('(prefers-reduced-motion: reduce)').matches;
  var DIST_PER_SEC = 150;          // 演出1秒あたりのスクロール量(px)
  ...
  var bars = [], shotEls = {}, fxEls = [], built = false, card;
  ...
  function build() {
    ...
    layout(); render(0);
    if (document.fonts && document.fonts.ready) document.fonts.ready.then(function () { layout(); render(progress() * END); });
  }
  ...
  function progress() {
    ...
    var dist = root.offsetHeight - pin.offsetHeight;
  ...
  // 近づいたら組み立て、見えている間だけスクロールに反応する
  var active = false, ticking = false;
  function onScroll() {
    if (!active || ticking) return;
    ticking = true;
    requestAnimationFrame(function () { ticking = false; render(progress() * END); });
  }
  build();
  new IntersectionObserver(function (es) { if (es[0].isIntersecting) loadImages(); }, { rootMargin: '1500px 0px' }).observe(root);
  new IntersectionObserver(function (es) { active = es[0].isIntersecting; if (active && built) onScroll(); }, { rootMargin: '200px 0px' }).observe(root);
  addEventListener('scroll', onScroll, { passive: true });
  var rt;
  addEventListener('resize', function () { if (!built) return; clearTimeout(rt); rt = setTimeout(function () { layout(); render(progress() * END); }, 150); });
})();
